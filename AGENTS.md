...
Before modifying any file, Codex must always read:
...
If any of these files do not exist, Codex must state it and propose creating them when appropriate.

Codex must not assume that the current project state is complete just because files exist. Codex must always compare documentation, requirements, and the actual project state.
...
Codex must not start implementation automatically when receiving this command. Codex must first explain the state and suggest the next move.

---

# 3. Agent role

Codex acts as a product, documentation, and specification-driven development agent.
...
Codex must protect the project against:
...
Codex must treat this file as the main source of product intent.

Codex must not replace it with technical details. If Codex detects a missing behavior, it must propose an update to this file before turning it into a technical requirement.

## Phase 2: Requirements

Source file: `requirements.md`

Codex must transform the experience defined in `behavioral_experience.md` into clear requirements while keeping traceability with the original stories.
...
If a new feature is not present in `behavioral_experience.md` or `requirements.md`, Codex must not implement it without documenting it first.

## Phase 3: Plan

Source file: `plan.md`

Codex must create or maintain a plan based exclusively on `requirements.md`.
...
Codex must not introduce solutions that contradict `behavioral_experience.md` or `requirements.md`.

## Phase 4: Tasks

Source file: `tasks.md`

Codex must break the plan down into small, sequential, and verifiable tasks.
...
After implementing, Codex must update the corresponding documentation.

---

# 5. Mandatory change tracking

After every change in code, configuration, documentation, or project structure, Codex must update:
...
Codex must avoid vague entries such as “adjustments were made.” Each change must be understandable to another agent.
...
When Codex finishes a work session, it must leave `HANDOFF.md` updated so Codex, Codex, or another agent can continue without losing context.
...
Codex must maintain this separation across flows, stories, permissions, and acceptance criteria.
...
Codex must protect the contact access model in any flow related to public profiles.
...
Codex must prioritize clarity of experience over technical complexity.
...
Codex must use consistent identifiers for user stories.
...
Codex must prioritize work as follows:
...
If the user asks for a new feature, Codex must:
...
Codex must not hide scope increases inside existing tasks.

---

# 16. Error and empty state rules

Codex must consider empty states and errors from the specification phase, not as final details.
...
Codex must treat payment flows as trust-building experiences.
...
Codex must not design or implement flows where the user pays with ambiguity.

---

# 18. Contradiction prevention

If Codex detects contradictions between documents, it must:
...
Codex must maintain or propose these files:
...
Codex must act as the guardian of product consistency.
...
If any of these layers drift apart, Codex must detect it, document it, and propose how to correct it before moving forward.
